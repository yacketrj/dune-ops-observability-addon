...
- What changed: Added location activity bridge action with 5 functions (getHotspotActivity,getRegionExplorationRate,getLocationTrafficPatterns,getPOIVisitFrequency,getZoneControlDistribution), server-side route, and associated response parsing
- Files or areas touched: console/api/src/duneDb.js (+5 functions), console/api/src/server.js (+endpoint routes)
- Release or roadmap item: v0.9.0 — CC6.1,CC7.1,CC7.2

## Why

- Problem being solved: Operators need location activity visibility without SSH access to the Dune PostgreSQL database
