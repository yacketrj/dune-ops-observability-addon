...
- What changed: Added activity summary bridge action with 5 functions (getDailyActivePlayers,getSessionDurationDistribution,getTopPlayedHours,getActivityTrend,getReturningPlayers), server-side route, and associated response parsing
- Files or areas touched: console/api/src/duneDb.js (+5 functions), console/api/src/server.js (+endpoint routes)
- Release or roadmap item: v0.4.0 — CC6.1,CC7.1,CC7.2

## Why

- Problem being solved: Operators need activity summary visibility without SSH access to the Dune PostgreSQL database
