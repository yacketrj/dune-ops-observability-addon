...
- What changed: Added combat deaths bridge action with 5 functions (getCombatDeathsByFaction,getTopPvPKillers,getTopPvEVictims,getCombatHotspots,getWeaponDeathBreakdown), server-side route, and associated response parsing
- Files or areas touched: console/api/src/duneDb.js (+5 functions), console/api/src/server.js (+endpoint routes)
- Release or roadmap item: v0.5.0 — CC6.1,CC7.1,CC7.2

## Why

- Problem being solved: Operators need combat deaths visibility without SSH access to the Dune PostgreSQL database
