...
- What changed: Added OPS health bridge bridge action with 6 functions (getOpsHealth,getActiveSessions,getDbPoolStats,getBridgeHealth,getOpsStatus,getRecentOpsLog), server-side route, and associated response parsing
- Files or areas touched: console/api/src/duneDb.js (+6 functions), console/api/src/server.js (+endpoint routes)
- Release or roadmap item: v0.3.0 — CC6.1,CC7.1

## Why

- Problem being solved: Operators need OPS health bridge visibility without SSH access to the Dune PostgreSQL database
...
output: Verified in clean-repro with unit tests (db.test.js)
```
