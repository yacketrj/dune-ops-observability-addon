...
- What changed: OPS health dashboard with session, pool, and bridge status cards
- Files or areas touched: addon.json, web/index.html (OPS section), web/addon.js (renderOpsHealth + renderActiveSessions + renderDbPoolStats + renderBridgeHealth + renderOpsStatus + renderRecentOpsLog), web/data-providers.js (OPS actions + sample data)
- Release or roadmap item: v0.3.0

## Why

- Problem being solved: Operators need OPS health dashboard with session, pool, and bridge status cards visualization in the Dune Console UI
...
- Release notes: Initial release of Dune Ops Observability addon. Requires core PR for v0.3.0 (OPS health bridge).
