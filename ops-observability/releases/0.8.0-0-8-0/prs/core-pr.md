...
- What changed: Added inventory summary bridge action with 5 functions (getInventoryDistribution,getRarestItems,getItemMarketValue,getInventoryTurnover,getGuildInventoryTotals), server-side route, and associated response parsing
- Files or areas touched: console/api/src/duneDb.js (+5 functions), console/api/src/server.js (+endpoint routes)
- Release or roadmap item: v0.8.0 — CC6.1,CC7.1,CC7.2

## Why

- Problem being solved: Operators need inventory summary visibility without SSH access to the Dune PostgreSQL database
